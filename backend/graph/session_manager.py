...
# Shared constant for compressed context prefix detection.
# Used by both session_manager and agent to identify summary messages.
...
class SessionManager:
    ...
    def clear_messages(self, session_id: str) -> None:
        """Clear all messages in a session but keep the session metadata."""
        data = self._read_file(session_id)
        if not data:
            return
        # Keep title and timestamps, only clear messages and compressed context
        data["messages"] = []
        if "compressed_context" in data:
