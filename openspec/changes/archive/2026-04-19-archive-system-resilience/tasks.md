...
- [ ] 1.1 验证 session_id 获取路径：`Runtime` 不含 config，改用 `get_config()` 从 LangGraph 上下文变量获取 `configurable.thread_id`（`backend/graph/middleware.py`）
...
- [ ] 1.3 修改 `abefore_model` 中调用 `_archive_output` 的位置，通过 `get_config()` 获取 session_id 并传入，获取失败时 fallback 为 `"unknown"`（`backend/graph/middleware.py`）
