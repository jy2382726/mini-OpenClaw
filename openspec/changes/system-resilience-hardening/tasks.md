...
- [ ] 2.2 修改 `get_auxiliary_model_config()` 函数末尾的默认值返回，改为引用 `_DEFAULT_CONFIG["auxiliary_model"]`（`backend/config.py:192`）
- [ ] 2.3 修改 `get_settings_for_display()` 中 `auxiliary_model` 缺失时的默认值，改为引用 `_DEFAULT_CONFIG["auxiliary_model"]`（`backend/config.py` 约第 286 行）
