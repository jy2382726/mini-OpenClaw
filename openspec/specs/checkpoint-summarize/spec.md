...
该端点 MUST 使用辅助 LLM（`create_auxiliary_llm()`）和 `DEFAULT_SUMMARY_PROMPT` 生成四段式摘要（SESSION INTENT / SUMMARY / ARTIFACTS / NEXT STEPS）。
...
手动摘要 MUST 使用 `DEFAULT_SUMMARY_PROMPT`（从 `langchain.agents.middleware.summarization` 导入）作为摘要 Prompt，确保手动和自动摘要的输出格式一致（SESSION INTENT / SUMMARY / ARTIFACTS / NEXT STEPS 四段结构）。
